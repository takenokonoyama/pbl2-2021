...
# サーバに対して直接通信する経路を調べる
def rooting_dir(my_port):
    ttl = 0
    client_socket = socket(AF_INET, SOCK_STREAM)
    client_socket.connect((server_name, mid_port)) # 送信するホストとコネクション
    # mid_port += 1
    # 送信するデータを配列に格納
    ...
    '''
    info_pack = [my_name, 'none', 'none',server_name , data, ttl, relay_num,'Root', 'req', my_port]
...
def SIZE_cmd(my_port, RootTable):
    # 直接SIZE要求
...
# GETコマンド
def GETall_cmd(client_socket, my_port, RootTable, token_str, server_file_name):
...
def GET_part_send(client_socket, my_port, RootTable, token_str, server_file_name, sep_data_s, sep_data_e, i):
...
# GETコマンド
def GET_part_cmd(my_port, RootTable, token_str, server_file_name, data_size):
    ...
    for i in range(0,len(RootTable)):
        ...
        # 送信用スレッド
        GETs = threading.Thread(target=GET_part_send, args=(client_socket, my_port, RootTable, token_str, \
...
# REPコマンド
def REP_cmd(my_port, RootTable, token_str, server_file_name):
    # 直接REP要求
    if(RootTable[0][2] == 'none'):
        client_socket = socket(AF_INET, SOCK_STREAM)
        client_socket.connect((server_name, server_port))
        ...
        print('SIZE_sentence', REP_sentence)
        return REP_sentence
...
if __name__ == '__main__':
    ...
    print('SIZE Command\n')
    my_port += 5
    data_size = SIZE_cmd(my_port, RootTable)
    print()
    ...
    # client_socket.connect((RootTable[0][2], mid_port)) # 送信するホストとコネクション
    # GETall
    # GET_sentence = GETall_cmd(client_socket, my_port, RootTable, token_str, server_file_name)
    # client_socket.close()

    start_time = time.time()
    # GETpartialコマンド
    GET_part_cmd(my_port, RootTable, token_str, server_file_name, data_size)
    ...
    REP_sentence = REP_cmd(my_port, RootTable, token_str, server_file_name)
    end_time = time.time()
