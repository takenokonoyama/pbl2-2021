...
def exchange_Routepacket(ad1, ad2, ttl, my_port):
    ...
    print("ad1, ad2, ttl",ad1, ad2, ttl)
    # 送信するデータを配列に格納
    data = f"abcdefghijklmnopqrstuvwxyz" # 任意のデータ
    # 参照する経由番号
    relay_num = 1
    '''
    パケット
    ...
    送信用(req)or応答用(rep), クライアントのポート番号,転送時間)
    '''
    info_pack = [my_name, ad1, ad2, server_name , data, ttl, relay_num, 'Route', 'req', my_port,0]
...
def routing_dir():
    ...
    try:
        ...
        rep_info_pack = future.result(timeout=2) # 応答の受け取り(タイムアウト時間の設定)
        # ルートテーブルへ調べた経路と時間を追加
    ...
    for ad in ADDRESS:
        ...
        if my_name != ad and server_name != ad:
            try:
                ...
                my_port += 1
                # futures.append(future)
                rep_info_pack = future.result(timeout=2)
                end_time = time.time()
